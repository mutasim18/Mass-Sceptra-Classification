...
def calculate_similarity_features(mz, int1, int2):
    ...
    try:
        ...
        # Return cosine similarity as a dictionary
        features = {
...
def generate_balanced_comparisons(base_path, debug=False):
        ...
        for i in range(1, 5):
            for j in range(i + 1, 6):
                ...
                if features is not None:
                    X.append(features['cosine_similarity'])  # Only use cosine similarity
    ...
    return np.array(X).reshape(-1, 1), np.array(y), comparisons
...
# Feature importance 
def plot_feature_importance(model, feature_names=['Cosine Similarity']):
    """Plots the importance of features."""
    importance = model.feature_importances_

    # Ensure feature_names matches the length of importance
    if len(feature_names) != len(importance):
        print("Warning: Feature names and importance lengths do not match!")
        print(f"Feature names provided: {len(feature_names)}")
        print(f"Feature importances available: {len(importance)}")
        # Adjust feature names to match importance length
    ...
    plt.title('Feature Importance for Cosine Similarity')
